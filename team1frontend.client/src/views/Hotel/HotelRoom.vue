    ...
    for (const id of ids) {
        try {
            ...
            const response = await axios.get(
                `https://localhost:7113/api/HotelCategories/${id}`
            );
...
onMounted(async () => {
    ...
    try {
        const hotelResponse = await axios.get(
            `https://localhost:7113/api/Hotels/${hotelId}`
        );
        const hotelData = hotelResponse.data;
        const roomsResponse = await axios.get(
            `https://localhost:7113/api/HotelRooms/hotel/${hotelId}`
        );
        ...
        hotel.value.hotelInfo = {
            ...
            checkInTime: hotelData.checkinStart
                .split(':')
                .slice(0, 2)
                .join(':'),
            checkOutTime: hotelData.checkoutEnd
                .split(':')
                .slice(0, 2)
                .join(':'),
            images: [
        ...
        hotel.value.roomTypes = roomTypesData.map((room) => ({
            ...
            facilities: room.roomFacilities.split(',').map(
                (
                    id // 假设 roomFacilities 是逗号分隔的设施 ID
                ) =>
